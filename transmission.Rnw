...
The code below implements Leydesdorff's $T$ (\cite{leydesdorff_routine_2012}) derived from Shannon's $H$ in $R$ (\cite{Rcore}). Leydesdorff's own implementation in another language is available at \url{http://www.leydesdorff.net/th}.
